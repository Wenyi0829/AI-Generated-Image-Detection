...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=False):
        ...
        # 可训练的多尺度NPR权重
        self.num_scales = 3
        self.npr_scales = [0.25, 0.5, 0.75]
        self.npr_weights = nn.Parameter(torch.ones(self.num_scales))
        self.npr_scale_factor = nn.Parameter(torch.tensor(2.0/3.0))  # 缩放因子也可训练
    ...
    def forward(self, x):
        ...
        for scale in self.npr_scales:
            NPR_scale = x - self.interpolate(x, scale)
            NPR_list.append(NPR_scale)

        normalized_weights = F.softmax(self.npr_weights, dim=0)

        NPR = torch.zeros_like(x)
        for i, (npr, weight) in enumerate(zip(NPR_list, normalized_weights)):
            NPR += weight * npr

        NPR = NPR * self.npr_scale_factor
        x = self.conv1(NPR)
...
######################################################################
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = x * self.ca1(x)  # channel attention
        x = self.maxpool(x)

        x = self.layer1(x)
        x = x * self.ca2(x) # channel attention
        x = self.layer2(x)

        # x = self.layer1(x)
        # x = self.spatial_att1(x)  # spatial attention
        # x = self.layer2(x)
        # x = self.spatial_att2(x)  # spatial attention
